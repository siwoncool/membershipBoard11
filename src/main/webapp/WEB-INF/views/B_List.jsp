...
<body>
  ...
  <main id="main">
      ...
      <div class="container" data-aos="fade-up">
        ...
        <div class="row">
          <div class="col-lg-12 text-center mb-5">
            <h1 class="page-title">${paging.boardCotegery} 게시판</h1>
          </div>
        </div>
	...
	<tr>
		...
		<td><a href="bView?boNum=${board.boNum}">${board.boTitle}</a></td>
		<td>${board.boDate}</td>
		<td>${board.boHit}</td>
	</tr>
	</c:forEach>
		...
		<tr>
			...
			<c:if test="${paging.page > 1}"><a href="bList?page=${paging.page - 1}">[이전]</a></c:if>
			
			<!-- [페이지번호] -->
			<c:forEach var="i" begin="${paging.startPage}" end="${paging.endPage}">
			
			<c:if test="${paging.page == i}">[${i}]</c:if>
			<c:if test="${paging.page != i}"><a href="bList?page=${i}">${i}</a></c:if>
			...
			<c:if test="${paging.page < paging.maxPage}"><a href="bList?page=${paging.page + 1}">[다음]</a></c:if>
